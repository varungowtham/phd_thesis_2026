...
Units can be written as $R=\SI{8.314}{\joule\per\mole\per\kelvin}$. 
		...
		\subsection{Equations}
			...
			\end{equation}							
		\subsection{Theorems/Definitions/Algorithms}	
			All of those can be defined using the amsth package in combination with the mdframed package for the frame. Use the protected space \textasciitilde{} between Def. ~and \verb+\ref+. An exemplary Definition is given in Def.~\ref{def:template_definition}. Equation~\ref{eq:template_equation_in_env} can also be referenced.
			
			Other environments, e.g.~for algorithms, can easily be added in the b\_Commands.tex file and get their own look.
			\begin{definition}[Stiff DAE systems]
