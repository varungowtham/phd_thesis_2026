...
%------------------------------		
	\section{Form}
		\begin{enumerate}
			...
			\item sentences should be comprehensible. Germans tend to formulate complex phrases with many subclauses. This should be avoided.
			...
			\end{itemize}
			The only correct use of squre brackets is shown here for the voltage: [U] = V, i.e.~the unit of the voltage is volt.
			...
			\item Figures, Tables, and Equations must be numerated and must be referenced in the text. This is automatically done using the caption package (see section \ref{sec:templates}) and the \verb+\ref+ command. For example, a Figure is named Figure chapter.Num (Fig. 2.1). The numeration is done automatically in this template.
			
			In addition, Figures and Tables must also be explained and discussed in the text.
			\item Figures should be chosen to support readabiliy and comprehension. The most important details and relevant labels, especially, must be \emph{readable}.
...
%------------------------------		
	\section{References and Bibliography}
		...
		\end{itemize}
		All information not generated by the author must be marked with a short reference, which is accompanied by the extensive reference in the bibliography. It is not important if this information appers directly or indirectly in the text.\\
		...
		In the following, a few examples for the authoryear short reference are stated. For more information, the reader is referred to the documentation of the biblatex package. Biblatex is used because it is compatible with UTF-8. Hence, Umlaute, such as ä, do not have to be rewritten as was the case in bibtex. Example: \parencite{Muller2018}, see .bib file.
		\begin{itemize}
...
%------------------------------			
	\section{Language: English or German?}\label{sec:language}
		...
		The language of the list of symbols is chosen via the option german in the nomentbl package. This also affects the entry in the table of contents. If the thesis is written in English, the option german can be removed.

		The name list of abbreviations must be changed in the respective file f\_abbrevi\-ations.tex as there is no package option available. The entry in the table of contents is changed in the same file.
		
		The language of the list of algorithms is chosen via the option german in the algorithmic2e package. This also affects the entry in the table of contents. If the thesis is written in English, the option german can be removed.

		If the language is English, it is recommended to change the output decimal marker  for SI units (siunitx package) to a dot. This can be changed in the a\_Packages.tex file where the siunitx package is loaded.
%------------------------------
% SEC: TEMPLATES
%------------------------------		
...
Units can be written as $R=\SI{8.314}{\joule\per\mole\per\kelvin}$. 
		...
		\subsection{Theorems, Lemmas, Proofs, Remarks, Definitions, and Algorithms}	
			...
			\paragraph*{Theorems:} Theorems can be defined using the amsthm package in combination with the mdframed package for a possible frame. An exemplary Theorem is given in Theorem~\ref{thm:template_theorem}.
				\begin{theorem_env}
					Let $f$ be a function whose derivative exists in every point, then $f$ is 
a continuous function.\label{thm:template_theorem}
				\end{theorem_env}			
			\paragraph*{Lemmas:} Lemmas can be defined using the amsthm package in combination with the mdframed package for a possible frame. An exemplary Lemma is given in Lemma~\ref{lem:template_lemma}.
				\begin{lemma_env}
				...
				\end{lemma_env}
 			\paragraph*{Proofs:} Proofs can be defined using the amsthm package. They are typically not numerated as they follow a certain Theorem or Lemma.
				\begin{proof}
					To prove it by contradiction, try and assume that the statemenet is false, proceed from there, and at some point, you will arrive at a contradiction.\label{pro:template_proof}
				\end{proof}	
			\paragraph*{Remarks:} Remarks can be defined using the amsthm package in combination with the mdframed package for a possible frame. An exemplary Remark is given in Remark~\ref{rem:template_remark}.	
				\begin{remark_env}
					This statement is true, I guess.\label{rem:template_remark}
				\end{remark_env}
			\paragraph*{Definitions:} Definitions can be defined using the amsthm package in combination with the mdframed package for a possible frame. An exemplary Definition is given in Def.~\ref{def:template_definition}. Equation~\ref{eq:template_equation_in_env} can also be referenced.		
				\begin{definition}
