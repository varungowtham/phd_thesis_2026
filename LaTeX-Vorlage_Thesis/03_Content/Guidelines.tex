...
Units can be written as $R=\SI{8.314}{\joule\per\mole\per\kelvin}$. 
		\subsection{Figures}
			A template for a Figure is given in Fig.~\ref{fig:template_figure}. Use the protected space \textasciitilde{} between Fig. ~and \verb+\ref+. Figure~\ref{fig:template_figure} should not be abbreviated at the beginning of a sentence.
			\begin{figure}[tbh]
				\centering
...
a continuous function.
			...
			\paragraph*{Algorithms:} Algorithms can be displayed using the algorithm2e package. An example is shown in Alg.~\ref{alg:template_algorithm}.\\
				...
				\end{algorithm}	
			\subsection{Autorefs}		
				The \verb+hyperref+ package also supplies an \verb+\autoref+ command that is linked to babel. In this case, Fig.~or Tab.~are localized and you do not have to worry what kind of float is referenced. As stated above, at the beginning of a sentence, the long version of an item should be used (the \verb+\Autoref{label}+ command was defined for this purpose):
				\begin{itemize}
