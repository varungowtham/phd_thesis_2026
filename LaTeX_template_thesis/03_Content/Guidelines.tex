...
\begin{enumerate}
	...
	\item check whether \verb+biber+\index{Scripts!biber} and \verb+makeindex+\index{Scripts!makeindex} were set up correctly. Detailed information on this matter can be found in \autoref{subsec:biblatexmakeindex}
	\item check whether you can compile this document without errors (see \autoref{subsec:compiling}). This should always be the case as long as all necessary packages are installed. This template was successfully compiled with a \TeX Live 2018 distribution (older \LaTeX\ distributions potentially contain older package version and are thus not supported anymore). \textbf{Some problems appeared when users did not have the newest versions of the used packages in this template. If you run into trouble, please update all your packages.\footnote{\url{https://tex.stackexchange.com/questions/55437/how-do-i-update-my-tex-distribution}, \\ January 2019}} See some more instructions for \TeX Live and MiK\TeX\ in \autoref{subsec:update_packages}
...
\begin{enumerate}
	...
	\item figures, tables, and equations must be numerated and referenced in the text. This is automatically done using the \verb+caption+\index{Packages!caption} package (see \autoref{sec:templates}) and the \verb+\autoref+\index{Commands!autoref@\verb+\autoref{}+} command (\autoref{subsec:autorefs}). For example, a Figure is named Figure chapter.Num (Fig. 2.1). The numeration is done automatically in this template. In addition, figures and  tables must also be explained and discussed in the text.
...
\begin{enumerate}
	...
	\item simple generation of List of References with the \verb+biblatex+\index{Packages!biblatex} package and \verb+biber+\index{Scripts!biber}
	\item DOI/ISBN/URL are automatically included in the List of References if they are available in the \path{.bib} file
	\item simple generation of an Index with the \verb+imakeidx+\index{Packages!imakeidx} package
	\item ToDos and missing figures with the \verb+todonotes+\index{Packages!todonotes} package
...
\begin{enumerate}
	...
	\item \path{c_Meta.tex}\index{Files!c\_Meta.tex}
	      ...
	      \end{itemize}
	\item \path{d_NomenclatureCommands.tex}\index{Files!d\_Nomenclature.tex}
	      \begin{itemize}
		      \item defines structure of the List of Symbols
		      \item optional argument defines the class of a symbol (Latin, Greek, \dots)
		      \item \verb+makeindex+\index{Scripts!makeindex} is used for the generation of the List of Symbols. A short instruction how to run \verb+makeindex+\index{Scripts!makeindex} correctly in Texmaker is given in \autoref{subsec:biblatexmakeindex}.
	      \end{itemize}
	...
	\item \path{f_CodeLanguageSpecifications.tex}\index{Files!f\_CodeLanguageSpecifications.tex}
	      \begin{itemize}
		      ...
		      \item if you want to apprehend your code in your thesis, you can either copy the code to an \verb+lstlistings+ environment (see examples in \autoref{ch:code_example_ampl} and \autoref{ch:code_example_matlab}) or  directly include your files
	      \end{itemize}
\end{enumerate}
\begin{figure}[tbh]
	\centering
...
The following files are located in this folder:
\begin{enumerate}
	\item \path{a_Cover_Diss.tex}\index{Files!a\_Cover\_Diss.tex} and \path{a_Cover_Thesis.tex}\index{Files!a\_Cover\_Thesis.tex}
	      \begin{itemize}
		      \item creates the cover page
		      \item \path{main.tex}\index{Files!main.tex} loads either \path{a_Cover_Thesis.tex} (cover of a bachelor/master thesis) or \path{a_Cover_Diss.tex} (cover of a dissertation), depending on the value of \verb+isDiss+ set in \path{c_Meta.tex}. Both covers are shown in \autoref{fig:covers}.
	      \end{itemize}
	\item \path{b_Declaration.tex}\index{Files!b\_Declaration.tex}
	      \begin{itemize}
		      \item declaration that the thesis was written honestly
	      \end{itemize}
	\item \path{c_Acknowledgements.tex}\index{Files!c\_Acknowledgements.tex}
	      \begin{itemize}
		      \item thank important people
	      \end{itemize}
	\item \path{d_Summary.tex}\index{Files!d\_Summary.tex}
	      \begin{itemize}
		      \item summarize your thesis in German and English
	      \end{itemize}
	\item \path{e_Nomenclature.tex}\index{Files!e\_Nomenclature.tex}
	      \begin{itemize}
		      \item enter all symbols and explain them
		      \item examples are provided
	      \end{itemize}
	\item \path{f_Abbreviations.tex}\index{Files!f\_Abbreviations.tex}
	      \begin{itemize}
...
\begin{enumerate}
	\item \path{a_Dedication.tex}\index{Files!a\_Dedication.tex}
	      \begin{itemize}
		      \item dedicate your dissertation to someone
	      \end{itemize}
	\item \path{b_Publications.tex}\index{Files!b\_Publications.tex}
	      \begin{itemize}
...
\begin{itemize}
	\item command line for setting up \verb+biber+\index{Scripts!biber} in Texmaker (see \autoref{fig:biblatexmakindex}):
	      \begin{enumerate}
		      \item Windows:\\ \verb+"C:/path_to/biber.exe" %+ \\
		            in \TeX Live, \verb+biber+\index{Scripts!biber} is located in \verb+bin/win32+
		      \item Linux:\\ \verb+"/usr/path_to/biber" %+
		      \item MacOS:\\ \verb+"/usr/path_to/biber" %.bcf+
	      \end{enumerate}
	\item command line for setting up \verb+makeindex+\index{Scripts!makeindex} in Texmaker (see \autoref{fig:biblatexmakindex}):
	      \begin{enumerate}
		      \item Windows:\\
		            {\footnotesize \verb+"C:/path_to/makeindex.exe" %.nlo -s nomencl.ist -o %.nls+}\\
		            in \TeX Live, \verb+makeindex+\index{Scripts!makeindex} is located in \verb+bin/win32+
		      \item Linux:\\
	      ...
	      \end{enumerate}
\end{itemize}	      
Note that \verb+path_to+ of course depends on your system. If the \LaTeX\ distribution is in your system path, simply writing \verb+biber+\index{Scripts!biber} or \verb+makeindex+\index{Scripts!makeindex} (without extension) instead of the whole path should suffice. If you use another editor than Texmaker,  check your editor's documentation to find out how to run \verb+biber+\index{Scripts!biber} and \verb+makeindex+\index{Scripts!makeindex} in this software. However, the paths/commands should be similar to those above.
\begin{figure}[tbh]
	\centering
	\includegraphics[width=\linewidth]{Settings_BiblatexMakeindex}
	\caption[Setting up \textbackslash\texttt{biber} and \textbackslash\texttt{makeindex} in Texmaker]{Setting up \textbackslash\texttt{biber} and \textbackslash\texttt{makeindex} in Texmaker. Zoom in if you need more details}\label{fig:biblatexmakindex}
\end{figure}
...
\begin{enumerate}
	...
	\item \verb+biber+\index{Scripts!biber}: Run \verb+biber+ (Texmaker standard short key: F11) after you set it up according to \autoref{subsec:biblatexmakeindex}. This creates the necessary temporary reference file.
	\item \verb+makeindex+\index{Scripts!makeindex}: Run \verb+makeindex+ (Texmaker standard short key: F12) after you set it up according to \autoref{subsec:biblatexmakeindex}. This creates the necessary temporary nomenclature file.
...
The local settings are important as they specify that only one backup is created. For more information, please refer to the documentation of \verb+latexindent+\index{Scripts!latexindent}. You can then execute your new user command in Texmaker with the opened file in question. Afterwards, update your file by clicking on File $\rightarrow$ Reload document from file. The result is shown in \autoref{fig:automatic_indenting_latexindent}. \textbf{Note that automatic indenting might not be available for online editors.}
...
\begin{mdframed}[linecolor=tub_red, roundcorner=10pt]
Currently (March 2019), there is an error message when using this template on Overleaf with the \verb+nomencl+\index{Packages!nomencl} and the \verb+scrhack+\index{Packages!scrhack} package, potentially because Overleaf uses \TeX Live 2017 at the moment. If you want to use this template on Overleaf, load the \verb+scrhack+ package with the option \verb+nomencl=false+. In addition, you need to upload a current version of the \verb+nomencl+ package (\path{.sty} file) to the Overleaf project folder. There is also an error due to the old version of the \verb+chemfig+\index{Packages!chemfig} package. On Overleaf, exchange the \verb+\setchemfig{...}+\index{Commands!setchemfig@\verb+\setchemfig{}+} command with \verb+\setatomsep{20pt}+\index{Commands!setatomsep@\verb+\setatomsep{}+}. These minor issues will disappear as soon as Overleaf updates to \TeX Live 2018 later this year.
\end{mdframed}
\begin{figure}[tbh]
	\centering
...
\subsection{Figures}
A template for a Figure is given in \autoref{fig:template_figure}. \Autoref{fig:template_figure} should not be abbreviated at the beginning of a sentence. Note that the often used \verb+\ref+\index{Commands!ref@\verb+\ref{}+} command is not used here. Instead, \autoref{subsec:autorefs} introduces the \verb+\autoref+\index{Commands!autoref@\verb+\autoref{}+} command. In addition, a short command for including figures was set up with the \verb+xparse+\index{Packages!xparse} package:
\begin{verbatim}
...
\end{wrapfigure}
Although it is not seen on a regular basis in theses or dissertations, a Figure may also be surrounded by text. The \verb+wrapfig+\index{Packages!wrapfig} package is used for this purpose. It depends on the Figure's size if this is a good or a bad idea. Make sure that readability of the Figure is still given. An example is given in \autoref{fig:wrapped_figure}.
\subsection{Tables}
\begin{wraptable}{r}{0.4\textwidth}
	...
	\end{tabular}
\end{wraptable}
A template for a Table is given in \autoref{tab:template_table}. \Autoref{tab:template_table} should not be abbreviated at the beginning of a sentence. A common issue in \LaTeX\ are footnotes within tables. However, there is the \verb+threeparttable+\index{Packages!threeparttable} package to deal with this. In such a \verb+threeparttable+ environment, annotations can be easily added (\autoref{tab:threeparttable}). Normal tables in \LaTeX\ cannot go over several pages. For longer tables, see the \verb+longtable+\index{Packages!longtable} package in \autoref{app:longtable}. This template also loads the \verb+threeparttablex+\index{Packages!threeparttablex} package, which extends the annotation feature to the \verb+longtable+ environment. The \verb+wrapfig+\index{Packages!wrapfig} package can also be used for wrapped tables as shown in \autoref{tab:wrapped_table}. Again, make sure that the Table is small enough.
\begin{table}[tbh]
	\centering
...
\end{itemize}
\myfigure[tbh][1]{Example_jabref}[Jabref]{Jabref}[fig:jabref]
\begin{figure}[tbh]
...
Sometimes, it is necessary to state chemical reactions or molecules. For this purpose, the \verb+chemfig+\index{Packages!chemfig} and the \verb+chemformula+\index{Packages!chemformula} package, which is loaded as part of the \verb+chemmacros+\index{Packages!chemmacros} package, are used as shown in \autoref{fig:template_chemical_reaction}. Molecular formulas should not be written in math mode, but can be typeset with the \verb+\ch{}+\index{Commands!ch@\verb+\ch{}+} command, e.g.~\ch{H2O}. Note that chemical formulae, e.g.~\ch{CO2} for carbon dioxide, \emph{should not} be added to the List of Symbols or the List of Abbreviations, while abbreviations, e.g.~\ch{MEA} for monoethanolamine, \emph{should} be added to the List of Abbreviations. Greek letters in chemicals are not typeset in italics, therefore the \verb+upgreek+\index{Packages!upgreek} package is loaded in order to typeset \iupac{\chemgamma-Aluminium} and not $\gamma$-Aluminium.
