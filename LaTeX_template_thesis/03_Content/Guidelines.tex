...
\begin{itemize}
	...
	\item command line for setting up makeindex in Texmaker (see \autoref{fig:biblatexmakindex}):
	      \begin{enumerate}
		      \item Windows:\\
		            {\footnotesize \verb+"C:/path_to/makeindex.exe" %.nlo -s mynomentbl.ist -o %.nls+}\\
		            in \TeX Live, makeindex is located in \verb+bin/win32+
		      \item Linux:\\ 
		      {\footnotesize \verb+"/usr/path_to/makeindex" %.nlo -s mynomentbl.ist -o %.nls+}
		      \item MacOS:\\ 
		      {\footnotesize \verb+"/usr/path_to/makeindex" %.nlo -s mynomentbl.ist -o %.nls+}
	      \end{enumerate}
