...
\begin{enumerate}
	...
	\item f\_CodeLanguageSpecifications
	      \begin{itemize}
		      ...
		      \item if you want to apprehend your code in your thesis, you can either copy the code to an lstlistings environment (see example in 05\_Appendix/1\_Appendix.tex) or  directly include your files
...
\end{lstlisting}
This means that \verb+latexindent+ is executed with writing rights (-w) on the current .tex file with local settings (-l) in the given path. These local settings are the only issue, because the absolute path can of course change if you move your folder. However, the relative path would always change depending on which file you actually want to indent.\\
The local settings are important as they specify that only one backup is created. For more information, please refer to the documentation of latexindent. You can then execute your user new command in Texmaker with the opened file in question. Afterwards. update your file by clicking on File $\rightarrow$ Reload document from file. The result is shown in \autoref{fig:automatic_indenting_latexindent}. \textbf{Note that automatic indenting might not be available for online editors.}
\begin{figure}[tbh]
	\centering
	\begin{subfigure}[t]{0.48\textwidth}
	...
	\end{subfigure}
	\caption{Before (left) and after (right) automatic indenting with latexindent}\label{fig:automatic_indenting_latexindent}
\end{figure}
%------------------------------
% SEC: TEMPLATES
%------------------------------		
