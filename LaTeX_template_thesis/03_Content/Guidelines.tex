...
\begin{enumerate}
	\item indent.yaml
	      \begin{itemize}
		      ...
		      \item more information can be found in \autoref{subsec:automatic_indenting}
	      \end{itemize}
...
\begin{figure}[tbh]
	...
	\end{minipage}
\end{figure}
\subsection{Automatic Indenting in Editor}\label{subsec:automatic_indenting}
...
Unfortunately, \LaTeX\ does not offer automatic indenting as do, for example, Matlab or Python. However, the perl script-based \verb+latexindent+ can be used for automatic indenting of the source code. This executable is part of every \LaTeX\ distribution. There are two ways of using \verb+latexindent+: directly running \verb+latexindent+ or calling it via \verb+arara+\footnote{\url{https://tex.stackexchange.com/questions/126241/autoindent-in-texmaker}, January 2019}. Only the second option is described here for a Windows operating system. It is expected that the instructions are similar on a Unix system, because \verb+arara+ is platform independent. Nevertheless, an issue are different versions of \verb+latexindtent+ and \verb+arara+. Therefore, instructions are given for version 3.0a of \verb+arara+ (\TeX Live 2017) and the current version 4.0 (\TeX Live 2018). These specific instructions are given in the next two paragraphs. If you have a \LaTeX\ distribution from before 2017, we recommend updating \LaTeX\ by getting the newest Mac\TeX, \TeX Live, or MiK\TeX\ version.
...
\textbf{Arara 4.0 (\TeX Live 2018):} In 2018, version 4.0 of \verb+arara+ was published. With this version, the indent rule is already part of the \LaTeX\ distribution and does not have to be added manually. The procedure is exactly the same as above. You can simply skip step 3 of the instructions. \textbf{Problem: Arara 4.0 seems to have an issue with subdirectories. Using it is currently not recommended.}
\begin{figure}[tbh]
	\centering
	\includegraphics[width=\linewidth]{arara_latexindent}
	\caption{Directives for arara execution for automatic indenting in the main.tex file}\label{fig:arara_latexindent}
\end{figure}
%------------------------------
