...
\begin{enumerate}
	...
	\item change the necessary entries in \path{b_Meta.tex}\index{Files!b\_Meta.tex} (e.g., your name, matriculation number, etc.), set the right value of the \verb+isDiss+ variable and the \verb+isMT+ variable (if it is a thesis) in this file
...
\begin{longtable}{p{0.25\textwidth}p{\mycolumnwidth}}
...
\endlastfoot
%
\path{main}\index{Files!main.tex} &
...
\multicolumn{2}{l}{\textbf{00\_Arara\_and\_Latexindent}} \\
\path{localSettings} & 
	\begin{minipage}[t]{\mycolumnwidth} \begin{myList}
	...
	\end{myList} \end{minipage} \\
\multicolumn{2}{l}{\textbf{01\_Document\_administration}} \\
\path{a_Packages}\index{Files!a\_Packages.tex} & 
	\begin{minipage}[t]{\mycolumnwidth} \begin{myList}
		\item All loaded packages
		\item Sorted based on application
		\item All packages are stated in \autoref{tab:package_list}
	\end{myList} \end{minipage} \\
\path{b_Meta}\index{Files!b\_Meta.tex} & 
	\begin{minipage}[t]{\mycolumnwidth} \begin{myList}
		...
		\item Boolean variable to select whether it is a master or a bachelor thesis
		\item Boolean variable to select whether the thesis contains an embargo notice due to a confidentiality agreement; also allows you to set the company's name
...
\path{a_Cover}\index{Files!a\_Cover.tex} & 
	...
	\end{myList} \end{minipage} \\
\path{b_Declaration}\index{Files!b\_Declaration.tex} &
	\begin{minipage}[t]{\mycolumnwidth} \begin{myList}
		\item Declaration that thesis was written honestly
	\end{myList} \end{minipage} \\    
\path{c_Dedication}\index{Files!c\_Dedication.tex} &
	\begin{minipage}[t]{\mycolumnwidth} \begin{myList}
		\item Dedication for thesis dissertation
		\item Only included if \verb+isDiss+ is true  
	\end{myList} \end{minipage} \\
\path{c_Task}\index{Files!c\_Task.pdf} &
	\begin{minipage}[t]{\mycolumnwidth} \begin{myList}
...
\begin{figure}[tbh]
	...
	\end{subfigure}
	\caption{Covers of this template. These were slightly adapted over the years with newer versions}\label{fig:covers}
\end{figure}
\subsection{Figures}
This folder is added to the \path{graphicspath} in the \path{c_Commands.tex} file and contains all figures in this template. Being added to the \path{graphicspath} means that you may include figures without having to specify a path. However, it might become a little confusing if all your figures are placed within one folder. Luckily, several different folders can be added to the \path{graphicspath}.
%------------------------------
% SEC: LANGUAGE
%------------------------------				
...
Dissertations are stored as PDF/A at the university library. To ensure compatibility of this template with PDF/A, the \verb+pdfx+\index{Packages!pdfx@\verb+pdfx+} package is used with the option \verb+a-2b+. The compiled PDF was successfully validated with Callas\footnote{\url{https://conversion.ub.tu-berlin.de/}, December 2018}, as recommended by the university library\footnote{\url{https://www.static.tu.berlin/fileadmin/www/10002444/Dokumente/Forschen_Publizieren/Dissertationsstelle/pdf-guide-202211-de.pdf}, October 2025 \label{fn:PDFA}}.  Note that contrary to the descriptions given in the linked document in \autoref{fn:PDFA}, a specific color profile is \emph{not} necessary if you use an up-to-date version of the \verb+pdfx+\index{Packages!pdfx@\verb+pdfx+} package as it automatically includes a free color profile. Check the documentation of the \verb+pdfx+\index{Packages!pdfx@\verb+pdfx+} package to find how to include other color profiles. Normally, the default should however suffice. The \verb+pdfx+\index{Packages!pdfx@\verb+pdfx+} package also loads the \verb+hyperref+\index{Packages!hyperref@\verb+hyperref+} package. \textbf{Note that the \texttt{pdfx} package cannot ensure that all your included PDFs fulfill the PDF/A standard. Hence, check your PDF for PDF/A compatibility whenever you included external files.}
...
Numbers and units are very important, and there are some rules when typesetting them. For example, units are never written in italics. They should also have the right space between them. For this purpose, the \verb+siunitx+\index{Packages!siunitx@\verb+siunitx+} package is suggested.
\begin{itemize}
	\item numbers (\verb+\num{number}+): \num{3.14159}
...
In many applications, optimization problems must be stated in theses and dissertations. This can be realized with the \verb+optidef+\index{Packages!optidef@\verb+optidef+} package:
\begin{mini}[2]
{\omega}
{f\left(\omega\right)}
{\label{prob:nlp}}{}
\addConstraint{0}{\geq g\left(\omega\right)}{\quad\text{(Constraints)}}
\addConstraint{\omega \in \Omega\,.}
\end{mini}
...
\end{itemize}
Another helpful method are shorthands. These can be used when the original author or institution would lead to a very long reference in the text, e.g., for norms. In the following, the normal reference and the shorthand citation are shown. The \verb+\textcite{}+\index{Commands!textcite@\verb+\textcite{}+} has been redefined to introduce the shorthand:
\begin{itemize}
...
All of these items are introduced in the following. Note that frames and their colors are arbitrary. You might want to change the color or the linewidth. You can do this in the \path{c_Commands.tex}\index{Files!c\_Commands.tex} file.
