...
\begin{figure}[tbh]
	...
	\end{minipage}
\end{figure}
\subsection{Automatic Indenting in Editor}
Indenting \emph{can} be used for structuring one's document, e.g.~by indenting everything within an equation environment. Unfortunately, \LaTeX\ does not offer automatic indenting as do, for example, Matlab or Python. However, the perl script-based \verb+latexindent+ can be used for automatic indenting of the source code. This executable is part of every \LaTeX\ distribution. There are two ways of using \verb+latexindent+: directly running \verb+latexindent+ or calling it via \verb+arara+\footnote{\url{https://tex.stackexchange.com/questions/126241/autoindent-in-texmaker}, January 2019}. Only the second option is described here for a Windows operating system. It is expected that the instructions are similar on a Unix system. An issue are different versions of \verb+latexindtent+ and \verb+arara+. Therefore, instructions are given for earlier versions of \verb+arara+ and the current version (4.0).
...
\begin{enumerate}[label={\arabic*{}.}]
	\item Add a user command for \verb+arara+ to Texmaker\footnote{\url{https://tex.stackexchange.com/questions/107989/integration-of-arara-in-texmaker}, January 2019}. \textbf{Due to system path issues}, is has been proven advantageous to put the complete path to \verb+arara+ in the this command:
	      \begin{verbatim}
"path_to_texlive\2017\bin\win32\arara.exe" %.tex
\end{verbatim}
	\item Add the following command to the beginning of the main.tex file (right before anything else):
	      \begin{lstlisting}[breaklines]
% arara: indent: { overwrite : yes, files : [ folder/file1.tex, folder/file2.tex ] , settings: local, where: Arara_and_Latexindent/Arara_30a/localSettings.yaml }
\end{lstlisting}
	      This command executes the automatic file indenting for all files (and files in subfolders) that are states within the square brackets.
	\item Add the indent.yaml file that is located in this template folder in Arara\_and\_Latexindent/Arara\_30a to your \verb+arara+ rules. These rules are typically located in "path\_to\_texlive/2017/texmf-dist/scripts/arara/rules". The .yaml file was taken from a github commit of the \verb+latexindent+ developer\footnote{\url{https://gist.githubusercontent.com/cereda/3ccdfb6257e5272ad9c1080a89c9ae9b/raw/9207c645f13d016686f90291073870a21f558a20/indent.yaml}, January 2019}. He repeatedly pointed out that they do not want to do much backwards compatibility. If you run into issues and this feature is \emph{really important} to you, think about updating your \LaTeX\ distribution (see next paragraph).
...
\textbf{Arara 3.0a (\TeX Live 2018):} In 2018, version 4.0 of \verb+arara+ was published. With this version, the indent rule is already part of the \LaTeX\ distribution and does not have to be added manually.
%------------------------------
