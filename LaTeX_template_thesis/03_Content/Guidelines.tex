...
\begin{enumerate}
	...
	\item check whether you can compile this document without errors (see \autoref{subsec:compiling}). This should always be the case as long as all necessary packages are installed. This template was successfully compiled with a \TeX Live 2018 and a \TeX Live 2019 distribution (older \LaTeX\ distributions potentially contain older package version and are thus not supported anymore). \textbf{Some problems appeared when users did not have the newest versions of the used packages in this template. If you run into trouble, please update all your packages.\footnote{\url{https://tex.stackexchange.com/questions/55437/how-do-i-update-my-tex-distribution}, \\ January 2019}} See some more instructions for \TeX Live and MiK\TeX\ in \autoref{subsec:update_packages}
...
\begin{enumerate}
	...
	\item figures, tables, and equations must be numerated and referenced in the text. This is automatically done using the \verb+caption+\index{Packages!caption@\verb+caption+} package (see \autoref{sec:templates}) and the \verb+\autoref+\index{Commands!autoref@\verb+\autoref{}+} command (\autoref{subsec:autorefs}). For example, a Figure is named Figure chapter.Num (Fig. 2.1). The numeration is done automatically in this template. In addition, figures and  tables must also be explained and discussed in the text.
...
\begin{enumerate}
	...
	\item simple generation of List of Algorithms with the \verb+algorithm2e+\index{Packages!algorithm2e@\verb+algorithm2e+} package and KOMA script
	\item simple generation of List of Codes with the \verb+listings+\index{Packages!listings@\verb+listings+} package and KOMA script
	\item simple generation of List of Symbols (Latin, Greek, etc.) with the \verb+nomencl+\index{Packages!nomencl@\verb+nomencl+} package
	\item simple generation of List of Abbreviations with the \verb+acro+\index{Packages!acro@\verb+acro+} package
	\item simple generation of List of References with the \verb+biblatex+\index{Packages!biblatex@\verb+biblatex+} package and \verb+biber+\index{Scripts!biber@\verb+biber+}
	\item DOI/ISBN/URL are automatically included in the List of References if they are available in the \path{.bib} file
	\item simple generation of an Index with the \verb+imakeidx+\index{Packages!imakeidx@\verb+imakeidx+} package
...
\begin{enumerate}
	\item \path{indent.yaml}
	      \begin{itemize}
		      \item contains the rule for arara\index{Scripts!arara@\verb+arara+} 3.0A
...
\begin{description}
	\item[\TeX Live] manages the packages via the tlmgr (\TeX Live 2018) or tlshell (\TeX Live 2019). The GUI of the tlmgr is located in \path{texlive/year/bin/win32}. In this folder, you find a file called \path{tlmgr-gui.exe} or \path{tlshell.exe}, respectively. Execute this program and the application shown in \autoref{fig:texlive_tlmgr} starts (the tlshell looks slightly different). Use this application to update your packages.
	\item[MiK\TeX] manages the packages in the MiK\TeX\ console (package manager in older versions), see \autoref{fig:miktex_console}. Use this application to update your packages.
\end{description}
...
Currently (March 2019), there is an error message when using this template on Overleaf with the \verb+nomencl+\index{Packages!nomencl@\verb+nomencl+} and the \verb+scrhack+\index{Packages!scrhack@\verb+scrhack+} package, potentially because Overleaf uses \TeX Live 2017 at the moment. If you want to use this template on Overleaf, load the \verb+scrhack+ package with the option \verb+nomencl=false+. In addition, you need to upload a current version of the \verb+nomencl+ package (\path{.sty} file) to the Overleaf project folder. There is also an error due to the old version of the \verb+chemfig+\index{Packages!chemfig@\verb+chemfig+} package. On Overleaf, exchange the \verb+\setchemfig{...}+\index{Commands!setchemfig@\verb+\setchemfig{}+} command with \verb+\setatomsep{20pt}+\index{Commands!setatomsep@\verb+\setatomsep{}+}. These minor issues will disappear as soon as Overleaf updates to \TeX Live 2018 or 2019 later this year.
\end{mdframed}
\begin{figure}[tbh]
	\centering
...
\subsection{Figures}
A template for a Figure is given in \autoref{fig:template_figure}. \Autoref{fig:template_figure} should not be abbreviated at the beginning of a sentence. Note that the often used \verb+\ref+\index{Commands!ref@\verb+\ref{}+} command is not used here. Instead, \autoref{subsec:autorefs} introduces the \verb+\autoref+\index{Commands!autoref@\verb+\autoref{}+} command. In addition, a short command for including figures was set up with the \verb+xparse+\index{Packages!@\verb+xparse+xparse} package:
\begin{verbatim}
...
\begin{definition}[What the definition defines]
	...
	\end{equation}
\end{definition}
\textbf{Algorithms:} Algorithms can be displayed using the algorithm2e\index{Packages!algorithm2e@\verb+algorithm2e+} package. An example is shown in \autoref{alg:template_algorithm}.\\
\begin{algorithm}[tbh]
	\SetAlgoLined
	\KwData{this text}
