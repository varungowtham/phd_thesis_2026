...
\begin{enumerate}
	...
	\item a List of Symbols and a List of Abbreviations must be included. This is done with the \verb+nomentbl+\index{Packages!nomentbl} and the \verb+acro+\index{Packages!acro} package in this template. In addition, symbols should be explained in the text after their first appearance. A List of Algorithms or other lists can be added if necessary.
...
\begin{itemize}
	...
	\item simple generation of List of Symbols (Latin, Greek, etc.) with the \verb+nomentbl+\index{Packages!nomentbl} package
...
\begin{itemize}
	...
	\item the necessary \path{mynomentbl.ist} file is located in the template folder
...
The only dependency Overleaf does not directly support is \verb+nomentbl+\index{Packages!nomentbl}. As this package needs makeindex\index{Scripts!makeindex}, you have to tell the Overleaf compiler how to proceed. This information is contained in the latexmkrc file in this template folder. Simply upload the latexmkrc file to the Overleaf project next to \path{main.tex} and Overleaf should compile this file correctly. In some cases, you may first have to clear the cache to remove old temporary files.\footnote{\url{https://www.overleaf.com/learn/how-to/Clearing_the_cache}, January 2019} In addition, make sure that you set \path{main.tex} as main document.
\begin{figure}[tbh]
	\centering
