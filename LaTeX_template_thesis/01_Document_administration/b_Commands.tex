...
%------------------------------
% ALPHANUMERICAL PAGE NUMBERS
%------------------------------
\makeatletter
\newcommand{\@AlphAlph}{}		% check that \@AlphAlph is undefined.
\let\@AlphAlph=\AlphAlph		% this allows for a counter, which goes from page Z to page AA
\makeatother
%------------------------------
% XPARSE
%------------------------------
% O is an optional argument and {} is the default, m ist the must-have argument
\NewDocumentCommand{\pder}{ O{} O{} m }{\frac{\partial^{#2}#1}{\partial#3^{#2}}}				% a short command for partial derivatives
...
\NewDocumentCommand{\ind}{m O{}}{\text{#1}\mathnormal{,#2}}			% a short command for indices, the first argument is printed as text, the second in math mode
%------------------------------
% CODE
%------------------------------
...
\iflanguage{ngerman}{
	\newcommand{\indextitle}{Indexregister}
}{
\iflanguage{english}{
	\newcommand{\indextitle}{Index Register}
}{}
}
\makeindex[
